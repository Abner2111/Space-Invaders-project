...
while True:
    #Proceso
    if GameRunnig:
        proceso(jugador,fps,frames_totales,sonido_jugador,sonido_enemigo)
        vida.enemigo()
        GameRunning= jugador.revisar_derrotado()

        frames_totales+=1
        screen.blit(background,(0,0))
        jugador.movimiento(SCREENWIDTH)#para que se pueda mover por la pantalla
        scoretex= pygame.font.Font('fonts/space_invaders.ttf', 28)("Puntos="+str(Naves.score),1,white)
        screen.blit(scoretex,(5,10))

        for enemigo in enemigo.lista:
            enemigo.movimiento(SCREENHEIGHT)

        General.allsprites.draw(screen)
        Proyectil.allproy.draw(screen)
        ...
        music.stop()
        scoretext=pygame.font.Font('fonts/space_invaders.ttf', 28)("Puntos="+str(Naves.score),1,white)
        screen.blit(scoretext, (SCREENWIDTH,40))
        pygame.display.flip()

        for sprite in General.allsprites:
            General.allsprite.remove(sprite)
            del sprite
        for proy in Proyectil.allproy:
            Proyectil.allproy.remove(proj)
            del proj
